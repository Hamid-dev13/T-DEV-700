...
export async function deleteUser(id) {
  ...
  const teams = getTeams().map(t => ({ ...t, members: t.members.filter(m => m !== id) }))
  localStorage.setItem(LS.users, JSON.stringify(users))
  localStorage.setItem(LS.teams, JSON.stringify(teams))
}

export async function addClock({ userId, type, timestamp }) {
  const clocks = getClocks()
  clocks.push({ id: id(), userId, type, timestamp: (timestamp instanceof Date ? timestamp.toISOString() : timestamp) })
  localStorage.setItem(LS.clocks, JSON.stringify(clocks))
}

export function getUserClocks(userId, fromISO, toISO) {
  const from = fromISO ? new Date(fromISO) : new Date('1970-01-01')
  const to = toISO ? new Date(toISO) : new Date('2999-12-31')
  return getClocks().filter(c => c.userId === userId && new Date(c.timestamp) >= from && new Date(c.timestamp) <= to)
}

export function computeDailyHours(events) {
  const byDay = {}
  ...
  for (const [day, arr] of Object.entries(byDay)) {
    ...
    for (let i=0;i<sorted.length;i+=2) {
      ...
      if (a && b && a.type === 'in' && b.type === 'out') {
        total += (new Date(b.timestamp) - new Date(a.timestamp)) / 3600000
      }
    }
...
export function aggregateWeekly(daily) {
  ...
  return Object.entries(byW).map(([week, hours]) => ({ week, hours: parseFloat(hours.toFixed(2)) }))
}

export function teamAverages(teamId, fromISO, toISO) {
  ...
  const daily = Object.entries(dAgg).map(([day, hours]) => ({ day, hours: parseFloat((hours/count).toFixed(2)) })).sort((a,b)=>a.day.localeCompare(b.day))
  const weekly = Object.entries(wAgg).map(([week, hours]) => ({ week, hours: parseFloat((hours/count).toFixed(2)) })).sort((a,b)=>a.week.localeCompare(b.week))
  return { daily, weekly }
}
