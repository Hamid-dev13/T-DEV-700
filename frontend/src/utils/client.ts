...
async function request(path: string, { method = "GET", headers = {}, payload, query }: { method?: string; headers?: Record<string, string>; payload?: any; query?: Record<string, any> } = {}, was_unauthorized = false) {
    ...
    if (res.status === 401) {
        // if unauthorized, try refreshing session before retrying

        if (!was_unauthorized) {
            try {
                await refreshSession()
                return await request(path, { method, headers, payload, query }, true)
            } catch (err) {
...
export async function refreshSession() {
    return request("/user/refresh", { method: "POST" }, true);  // set was_unauthorized to true to avoid infinite loop
}
