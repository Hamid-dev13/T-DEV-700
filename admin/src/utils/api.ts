...
export async function getTeamUsers(teamId: string): Promise<{ manager: User; members: User[] }> {
  return get(`/teams/${encodeURIComponent(teamId)}/users`)
}

export async function addTeam(name: string, description: string, start_hour: number, end_hour: number, manager: string) {
